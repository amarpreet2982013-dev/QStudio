...
import { assertSourceSize } from "./inputLimits";
...
export class SilqCompiler implements SilqCompilerContract {
  ...
  async analyze(source: string): Promise<CompilationResult> { assertSourceSize(source); const tokens = this.lexer.tokenize(source); const parsed = this.parser.parse(tokens); const semantic = this.analyzer.analyze(parsed.ast); const generated = this.irGenerator.generate(parsed.ast, semantic); const diagnostics = [...parsed.diagnostics, ...semantic.diagnostics, ...generated.diagnostics]; return { tokens, ast: parsed.ast, semantic, ir: generated.ir, qasm: this.qasmGenerator.generate(generated.ir), circuit: this.circuitGenerator.generate(generated.ir), diagnostics }; }
